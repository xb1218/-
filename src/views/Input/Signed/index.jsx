...
export default function Record() {
  ...
  const getPatientInfo = async () => {
    const res = await api.get("/patient/basic", { pid, type: "test" })
    setInfo(res)
    setImg(res.imgUrl)
  }
  ...
  const onSave = async (labels, date) => {
    ...
    const res = await api.post("/add/addTestSheet", params, {
      success: "新增化验单成功",
    })
    setImg("")
    return true
            ...
            <PosBottom>
              <Space size={32}>
                <Button
                  type="primary"
                  onClick={() =>
                    history.replace(`/home/input/signed-all/${pid}`)
                  }
                >
                  总览
                </Button>
